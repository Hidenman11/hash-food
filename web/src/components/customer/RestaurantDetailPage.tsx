...
export function RestaurantDetailPage({ slug }: RestaurantDetailPageProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([getRestaurantDetails(slug), getRestaurantMenu(slug)])
      .then(([details, items]) => {
        if (!active) return;
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="mt-6 space-y-6">
            <div className="h-40 animate-pulse rounded-2xl border border-white/[0.08] bg-[#0c1119]" />
            <div className="grid gap-4 md:grid-cols-2">
              {Array.from({ length: 4 }).map((_, index) => (
                <div
                  key={index}
                  className="h-28 animate-pulse rounded-2xl border border-white/[0.08] bg-[#0c1119]"
                />
              ))}
            </div>
          </div>
          ...
          <>
            <div className="mt-6 rounded-2xl border border-white/[0.08] bg-[#0c1119] p-6 sm:p-8">
              ...
              <div className="mt-5 flex flex-wrap gap-3 text-sm text-zinc-400">
                <span className="rounded-xl bg-white/[0.06] px-3 py-2">{restaurant.city}</span>
                {restaurant.address ? (
                  <span className="rounded-xl bg-white/[0.06] px-3 py-2">{restaurant.address}</span>
                ) : null}
                <span className="rounded-xl bg-white/[0.06] px-3 py-2">
                  Delivery {formatTzs(restaurant.deliveryFeeTzs)}
                </span>
                <span className="rounded-xl bg-white/[0.06] px-3 py-2">{restaurant.menuCount} items</span>
              </div>
            </div>
              ...
              {menu.length ? (
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                  {menu.map((item) => (
                    <article
                      key={item.id}
                      className="rounded-2xl border border-white/[0.08] bg-[#0c1119] p-5"
                    >
